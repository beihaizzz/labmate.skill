...
LabMate 兼容以下 AI 编程助手：
- **OpenCode** — 完整支持 `/lab` 命令 + visual model 图片分析
- **Claude Code** — `/lab` 命令 + Claude Vision 多模态图片分析
- **Cursor** — 通过 `.cursor/rules` 注入 skill 内容

> 💡 **视觉模型建议**：如果你的 AI 助手支持多模态视觉模型（如 Claude Vision、GPT-4o 等），LabMate 可以自动分析实验截图/照片内容，识别其中的代码、接线、仪表读数等信息，极大提升报告生成质量。无视觉模型时将自动使用图片占位标记。
...
| 风格 | 说明 |
|------|------|
| **标准（normal）** | **日常首选**。标准实验报告，内容完整规范，90+ 分水平 |
| **完美（perfect）** | **特殊场景**。极尽详尽，覆盖所有细节 |
